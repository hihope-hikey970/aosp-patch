...
public class WificondScannerImpl extends WifiScannerImpl implements Handler.Callback {
    ...
    public WifiScanner.ScanData getLatestSingleScanResults() {
        Log.d("ZLJ", TAG + "---" + "getLatestSingleScanResults:" + Arrays.asList(mLatestSingleScanResult.getResults()));
    ...
    private void pollLatestScanData() {
        ...
        Log.d("ZLJ", TAG + "---" + "pollLatestScanData");
        ...
        synchronized (mSettingsLock) {
            ...
            for (int i = 0; i < mNativeScanResults.size(); ++i) {
                ...
                Log.d("ZLJ", TAG + "---" + "pollLatestScanData:" + result.SSID +
                    "{" + timestamp_ms + "-" + mLastScanSettings.startTime + "=" + (timestamp_ms - mLastScanSettings.startTime) + "}");

                if (timestamp_ms < mLastScanSettings.startTime) {
                   result.timestamp = (mLastScanSettings.startTime + 10) * 1000;
                   timestamp_ms = result.timestamp / 1000;
                }
                ...
                if (timestamp_ms > mLastScanSettings.startTime) {
                    if (mLastScanSettings.singleScanFreqs.containsChannel(
                        ...
                        Log.d("ZLJ", TAG + "---" + "pollLatestScanData 1111:" + result.SSID);
                    }
                } else {
                    Log.d("ZLJ", TAG + "---" + "pollLatestScanData 2222:" + result.SSID);
                    numFilteredScanResults++;
                }
            }
            if (numFilteredScanResults != 0) {
                Log.d(TAG, "Filtering out " + numFilteredScanResults + " scan results.");
                Log.d("ZLJ", TAG + "---" + "Filtering out " + numFilteredScanResults + " scan results.");
